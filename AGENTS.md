...
- **工具输出窗口化压缩 (Tool Output Windowing)**：对单次超过 150 行的终端/日志/检索原始输出实施头尾窗口截断，仅提炼 3 行事实摘要，维持主上下文低水位并最大化 Gemini Prompt 前缀缓存（Prefix Caching）命中率。
...
- **长程窗口化自压缩 (Auto-Compaction)**：多轮长会话中（>20 轮），对工具输出强制执行头尾窗口截断，维持主上下文低水位并最大化 Gemini 前缀缓存（Prompt Caching）命中率。
