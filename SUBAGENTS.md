...
在 Antigravity 智能体工程体系中，**严禁同一 Agent 自编自审**（自审极易产生“作者认知偏见”，遗漏严重并发死锁与边界逻辑缺陷）。必须严格划分为**执行轨 (Executive Track)** 与 **对抗审查轨 (Audit Track)**：
...
```json
{
  ...
  "Workspace": "inherit",
  "Prompt": "脱离编写者先验偏见，仅针对本次 Git Diff 进行独立对抗审查：1. 注入高并发死锁、竞态条件与资源泄露测试；2. 反向挑刺未捕获的边界异常与安全越权漏洞；3. 输出带严重级别 (P0/P1/P2) 的客观审计报告。"
}
```
